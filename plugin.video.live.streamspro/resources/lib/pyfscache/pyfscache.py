...
class FSCache(object):
  ...
  def _remove(self, k,kisdigested):
    """
    ...
    so don't use it as part of the API.
    """
    """if using .getloaded() k is already digest."""
    if not kisdigested:
        digest = make_digest(k)
    else:
        digest = k
    ...
    if xbmcvfs.exists(path):
      ...
      msg = "No object for key `%s` stored." % str(path)
      raise CacheError, msg
  def is_loaded(self, k,kisdigested=False):
    """
